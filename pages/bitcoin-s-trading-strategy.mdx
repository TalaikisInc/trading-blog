...
export async function getStaticProps() {
  const res = await fetch('https://api.talaikis.com/v1.0/stats/s')
...
Bitcoin S is quantitative trading strategy in Bitcoin, which uses [quantitative trading models](/posts/trading-philosophy) for trading decisions making.
